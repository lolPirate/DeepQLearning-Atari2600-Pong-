...
class Agent:
    ...
    def learn(self):
        ...
        V_new_s, A_new_s = self.q_next.forward(new_states)
        V_new_s_eval, A_new_s_eval = self.q_eval.forward(new_states)
        

        q_pred = T.add(V_s, (A_s - A_s.mean(dim=1, keepdim=True)))[indices, actions]
        q_next = T.add(V_new_s, (A_new_s - A_new_s.mean(dim=1, keepdim=True)))

        q_eval = T.add(V_new_s_eval, (A_new_s_eval - A_new_s_eval.mean(dim=1, keepdim=True)))
